...
:pushpin: City informations gathered from (https://openweathermap.org/api/geocoding-api)

:pushpin: Air pollution informations gathered from (https://openweathermap.org/api/air-pollution#history)

:pushpin: Air pollution categories classify that according to table of “AQI Category,
Pollutants and Health Breakpoints”
from (https://en.wikipedia.org/wiki/Air_quality_index#CAQI)

:pushpin: Air pollution data only contains Carbon monoxide (CO), Ozone (O3), Sulphur dioxide (SO2) 

:pushpin: User can request query with city name and date range 

:pushpin: Previously existing records in the database can be deleted.

:pushpin: If the desired date is available in the database, it was taken from the database. If it is not in the database, it is saved to the database via the API.

:pushpin: City name is must. Date range is default one week range.

:pushpin: Using a historical data access API, from November 27, 2020, to the present
The date range was discovered to be wrong for entries made outside of these dates, and an error message was provided.

:pushpin: Using the log mechanism, it is recorded whether the information given is from the database or API.

:pushpin: The results were returned in Json format.
...
## MAKE A REQUEST CITY AIR POLLUTION INFORMATION BY REQUESTED DATE RANGE 
...
## MAKE A REQUEST CITY AIR POLLUTION INFORMATION BY WRONG DATE RANGE 
...
## MAKE A REQUEST CITY AIR POLLUTION INFORMATION BY WRONG DATE RANGE 2
